...
void AbstractGenerator::generateEntityDelegate(const QStringList& tbNames) {
...
void AbstractGenerator::generateConfigureFile(const QStringList& tbNames) {
    //output pri configure file
    QString priContent = "# ----------------------------------------------------\n"
    ...
    priContent += "HEADERS += ";
    for (const auto& name : tbNames) {
        priContent += QString("./%1.h \\\n    ").arg(name);
    }
    priContent += QString("./%1EntityInclude.h").arg(getSqlTypeName());
    priContent += QString("\nSOURCES += ./%1EntityInclude.cpp\n").arg(getSqlTypeName());

    writeContentWithCheckHash(priContent, getOutputFilePath("entity.pri"));
    ...
    cmakeContent += "set(ENTITY_FILE_LIST\n";
    cmakeContent += QString("    ${CMAKE_CURRENT_LIST_DIR}/%1EntityInclude.h\n    ${CMAKE_CURRENT_LIST_DIR}/%1EntityInclude.cpp\n\n").arg(getSqlTypeName());
    for (const auto& name : tbNames) {
        cmakeContent += QString("    ${CMAKE_CURRENT_LIST_DIR}/%1.h\n").arg(name);
    }
    cmakeContent += ")";

    writeContentWithCheckHash(cmakeContent, getOutputFilePath("entity.cmake"));
}
